...
function logout(){auth.logout();router.push('/login')}onMounted(()=>auth.refresh().catch(()=>auth.logout()))
</script>
<template>
  ...
  <div v-else class="app-shell">
    ...
    <nav v-if="auth.loggedIn" class="mobile-nav"><router-link v-for="item in nav.slice(0,5)" :key="item.to" :to="item.to"><el-icon><component :is="item.icon"/></el-icon><span>{{item.label}}</span></router-link></nav>
  </div>
</template>
